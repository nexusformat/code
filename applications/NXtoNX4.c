/*-----------------------------------------------------------------------------
  ...
  Utility to convert a NeXus file into HDF4
...
/* use command line arguments for input and output filenames if given */
   ...
   if (argc < 3) {
      printf ("Give name of output NeXus 4 file : ");
...
/* Open NeXus input file and NeXus 4 output file */
   if (NXopen (inFile, NXACC_READ, &inId) != NX_OK) {
      printf ("NX_ERROR: Can't open %s\n", inFile);
      return NX_ERROR;
   }

   if (NXopen (outFile, NXACC_CREATE4, &outId) != NX_OK) {
      printf ("NX_ERROR: Can't open %s\n", outFile);
      return NX_ERROR;
   }
