/*
 * $Id: SGTLineLayout.java,v 1.2 2009/10/08 08:47:37 koennecke Exp $
 *
 ...
 * 
 * This is a cut down version of the original noaaa JPlotLayout for 
 * LinePlots only.
 *
 ...
 *
 * @author Donald Denbo
 * @version $Revision: 1.2 $, $Date: 2009/10/08 08:47:37 $
 * @since 2.0
...
**/
public class SGTLineLayout extends JGraphicLayout
  ...
  private static final String YID = "YAxisID"; 
  private Axis savedYAxis;
   /**
  ...
  public boolean isYlog() {
		...
		public void setYlog(boolean ylog) {
			...
			AxisTransform yt = graph.getYTransform();
			try{
				yleft = savedYAxis;
			}catch(Exception e){
				return;
			...
			if(ylog){
				Range2D physRange = yt.getRangeU();
				physRange.start = Math.max(physRange.start, .2);
				LogTransform newYT = new LogTransform(yt.getRangeP(), physRange);
				LogAxis newyleft = new LogAxis(LEFT_AXIS);
			    newyleft.setRangeU(yt.getRangeU());
			    newyleft.setNumberSmallTics(0);
			    newyleft.setLabelHeightP(labelHeight_);
			    newyleft.setLocationU(yleft.getSoTLocationU());
			    newyleft.setLabelFont(yleft.getLabelFont());
			    graph.setYTransform(newYT);
			    graph.removeAllYAxes();
			    graph.addYAxis(newyleft);
			    savedYAxis = newyleft;
			} else {
				LinearTransform newYT = new LinearTransform(yt.getRangeP(), yt.getRangeU());
				PlainAxis newyleft = new PlainAxis(LEFT_AXIS);
			    newyleft.setRangeU(yt.getRangeU());
			    newyleft.setNumberSmallTics(0);
			    newyleft.setLabelHeightP(labelHeight_);
			    newyleft.setLocationU(yleft.getSoTLocationU());
    ...
    if(data_.size() == 0) {
      ...
      if(data_good) {
        ...
        origin = new SoTPoint(xnRange.getStart(), ynRange.getStart());
    	  ...
    	  System.out.println("Dada is bad");
      }
     ...
     //
      ...
      if(isOverlayed()) {
        ...
        // transforms
        //
        xt = (AxisTransform)graph.getXTransform();
        yt = (AxisTransform)graph.getYTransform();
        try {
          bottom = graph.getXAxis(BOTTOM_AXIS);
          left = graph.getYAxis(LEFT_AXIS);
        } catch (AxisNotFoundException e) {}
  ...
  public Domain getRange() {
    ...
    AxisTransform xt = (AxisTransform)graph.getXTransform();
    AxisTransform yt = (AxisTransform)graph.getYTransform();
    Range2D xr = null;
    Range2D yr = null;
      xr = xt.getRangeU();
      yr = yt.getRangeU();
...
//    warnHeight_ = second;
  ...
  public void linekeyVisible(boolean b){
	lineKey_.setVisible(b);
  }
 }
