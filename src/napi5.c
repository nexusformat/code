...
NXstatus NX5putdata(NXhandle fid, const void *data)
{
	...
	if (unlimiteddim) {
		...
		iRet =
		    H5Dwrite(pFile->iCurrentD, pFile->iCurrentT, H5S_ALL,
...
NXstatus NX5putslab64(NXhandle fid, const void *data, const int64_t iStart[],
		      const int64_t iSize[])
{
	pNexusFile5 pFile;
	int iRet, rank, i;
	...
	hsize_t size[H5S_MAX_RANK], thedims[H5S_MAX_RANK],
	    maxdims[H5S_MAX_RANK];
	...
	if (unlimiteddim) {
		...
		/* define slab */
		iRet =
		    H5Sselect_hyperslab(pFile->iCurrentS, H5S_SELECT_SET,
					myStart, NULL, mySize, NULL);
		/* deal with HDF errors */
		if (iRet < 0) {
			NXReportError("ERROR: selecting slab failed");
			return NX_ERROR;
...
NXstatus NX5makenamedlink(NXhandle fid, CONSTCHAR * name, NXlink * sLink)
{
	...
	H5Lcreate_hard(pFile->iFID, sLink->targetPath, H5L_SAME_LOC, linkTarget,
		       H5P_DEFAULT, H5P_DEFAULT);
...
herr_t nxgroup_info(hid_t loc_id, const char *name, const H5L_info_t * statbuf,
		    void *op_data)
{
...
herr_t group_info1(hid_t loc_id, const char *name, const H5L_info_t * statbuf,
		   void *opdata)
{
	int iNX = *((int *)opdata);
	H5O_info_t object_info;
				 ...
				 NXname pClass)
{
	...
	if (pFile->iCurrentG == 0) {
		...
		H5Literate(grp, H5_INDEX_NAME, H5_ITER_INC, 0, group_info1,
			   &pFile->iNX);
		H5Gclose(grp);
		*iN = pFile->iNX;
	} else {
		...
		if (attr_id < 0) {
			...
			pFile->iNX = 0;
			grp =
			    H5Gopen(pFile->iFID, pFile->name_ref, H5P_DEFAULT);
			H5Literate(grp, H5_INDEX_NAME, H5_ITER_INC, 0,
				   group_info1, &pFile->iNX);
			H5Gclose(grp);
			*iN = pFile->iNX;
			H5Aclose(attr_id);
		}
			 ...
			 int *datatype)
{
	...
	iRet =
	    H5Literate(grp, H5_INDEX_NAME, H5_ITER_INC, &idx, nxgroup_info,
		       &op_data);
	H5Gclose(grp);
...
NXstatus NX5getdata(NXhandle fid, void *data)
{
	...
	if (H5Tis_variable_str(pFile->iCurrentT)) {
		...
		H5Dvlen_reclaim(memtype_id, pFile->iCurrentS, H5P_DEFAULT,
				vstrdata);
		free(vstrdata);
		H5Tclose(memtype_id);
...
NXstatus NX5getslab64(NXhandle fid, void *data, const int64_t iStart[],
		      const int64_t iSize[])
{
	pNexusFile5 pFile;
	hsize_t myStart[H5S_MAX_RANK];
...
herr_t attr_info(hid_t loc_id, const char *name, const H5A_info_t * unused,
		 void *opdata)
{
...
NXstatus NX5getgroupID(NXhandle fileid, NXlink * sRes)
{
	...
	if (pFile->iCurrentG == 0) {
		...
		memset(sRes->targetPath, 0, datalen * sizeof(char));
		if (NX5getattr
		    (fileid, "target", sRes->targetPath, &datalen,
		     &type) != NX_OK) {
			buildCurrentPath(pFile, sRes->targetPath, 1024);
		}
		NXMEnableErrorReporting();
...
NXstatus NX5nativeexternallink(NXhandle fileid, const char *name,
			       const char *externalfile,
			       const char *remotetarget)
{
	herr_t iRet;
	pNexusFile5 pFile;
	hid_t openwhere;
	...
	iRet =
	    H5Lcreate_external(externalfile, remotetarget, openwhere, name,
			       H5P_DEFAULT, H5P_DEFAULT);
	if (iRet < 0) {
...
NXstatus NX5nativeinquirefile(NXhandle fileid, char *externalfile,
...
NXstatus NX5nativeisexternallink(NXhandle fileid, const char *name, char *url,
				 const int urllen)
{
	pNexusFile5 pFile;
	herr_t ret;
	...
	ret =
	    H5Lunpack_elink_val(linkval_buff, val_size, NULL, &filepath,
				&objpath);
	if (ret < 0) {
...
NXstatus  NX5getnextattra(NXhandle handle, NXname pName, int *rank, int dim[], int *iType)
{
	...
	if (intern_idx > idx) {
		iRet = H5Aiterate(vid, H5_INDEX_CRT_ORDER, H5_ITER_INC, &idx,
			       attr_info, &iname);
	} else {
		iRet = 0;
	}
